...
import { Title, EmptyStateBody, Bullseye, EmptyState, EmptyStateVariant } from '@patternfly/react-core';
import { Table as PfTable, TableBody, TableHeader, TableGridBreakpoint, cellWidth, TableVariant, sortable } from '@patternfly/react-table';
...
class EntityTable extends React.Component {
    ...
    createColumns = () => {
        ...
        return columns.map(({ props, transforms, ...oneCell }) => ({
            ...oneCell,
            transforms: [...(this.buildTransforms(props, transforms, hasItems, rows) || [])]
        }));
    };

    render() {
        const { rows, columns, loaded, expandable, onExpandClick, hasCheckbox, actions, variant, sortBy, tableProps } = this.props;
                ...
                {loaded ? (
                    PfTable && (
                        ...
                        >
                            <TableHeader />
                            <TableBody
                                onRowClick={(event, { selected }, { rowIndex }) => {
