...
export const mapData = ({ facts = {}, ...oneResult }) => ({
    ...
    facts: {
        ...
        ...flatMap(facts, (oneFact) => Object.values(oneFact))
            ...
            .reduce((acc, curr) => ({ ...acc, ...(typeof curr !== 'string' ? curr : {}) }), {})
    }
});
...
export const constructTags = (tagFilters) => {
    return (
        flatMap(tagFilters, ({ values, category: namespace }) =>
            values.map(({ value: tagValue, tagKey }) => `${namespace ? `${namespace}/` : ''}${tagKey}${tagValue ? `=${tagValue}` : ''}`)
        ) || ''
    );
};
...
export function getEntities(items, { controller, hasItems, filters, per_page: perPage, page, orderBy = 'updated', orderDirection = 'DESC' }) {
    const { hostnameOrId, tagFilters, staleFilter } = filters ? filters.reduce(filtersReducer, {}) : {};
    if (hasItems && items.length > 0) {
        return hosts
            .apiHostGetHostById(items, undefined, perPage, page, undefined, undefined, { cancelToken: controller && controller.token })
            .then(mapTags)
            .then(({ results = [], ...data } = {}) => ({
                ...data,
                filters,
...
export function getAllTags(search, { filters } = {}) {
    ...
    return tags.apiTagGetTags(tagFilters ? constructTags(tagFilters) : undefined, undefined, undefined, 10, undefined, undefined, search);
}
