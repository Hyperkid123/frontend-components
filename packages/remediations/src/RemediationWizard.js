...
function createNotification(id, name, isNewSwitch) {
    ...
    return {
        ...
        description: (
            <span>
                You have succesfully {verb} <a href={remediationUrl(id)}>{name}</a>.
            </span>
        ),
        dismissable: true
    };
...
class RemediationWizard extends Component {
    ...
    loadResolutions = async (issues) => {
        try {
            ...
            const [resolutions, errors] = transform(
                result,
    ...
    createRemediation = (add, resolver) => {
        ...
        return api
            .createRemediation({ name, add, auto_reboot: this.state.autoRebootSwitch }, this.state.open.basePath)
    ...
    updateRemediation = (add, resolver) => {
        ...
        return api
            .patchRemediation(id, { add, auto_reboot: this.state.autoRebootSwitch }, this.state.open.basePath)
    ...
    onResolutionSwitch = (issueId, resolutionId) =>
        this.setState({
            selectedResolutions: {
    ...
    buildSteps = () => {
        ...
        if (this.state.manualResolutionSelection) {
            steps.push(
                ...multiResolutionIssues.map((issue) => (
        ...
        steps.push(
            <SummaryStep key="SummaryStep" state={this.state} getResolution={this.getResolution} onAutoRebootSwitch={this.onAutoRebootSwitch} />
        );
