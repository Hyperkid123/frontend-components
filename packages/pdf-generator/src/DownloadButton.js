...
class DownloadButton extends React.Component {
    render() {
        const { pages, fileName, label, type, style, title, isPreview, reportName, size, ...props } = this.props;
                    ...
                    <Page size={size} key={key} style={[appliedStyles.page, appliedStyles.text]}>
                        {
                            <React.Fragment>
                                ...
                                <View>
                                    <Text style={appliedStyles.displayFont}>{customTitle(title)}</Text>
                                </View>
                            </React.Fragment>
                        }
                        <View style={appliedStyles.smallSpacing}>{page}</View>
                        <View
                            style={[
        ...
        return (
            <React.Fragment>
                {isPreview ? (
                    ...
                    <PDFDownloadLink document={document} fileName={fileName} {...props}>
                        {label}
                    </PDFDownloadLink>
                )}
            </React.Fragment>
...
DownloadButton.propTypes = {
    pages: PropTypes.arrayOf(PropTypes.oneOfType([PropTypes.node, PropTypes.arrayOf(PropTypes.node)])),
    size: PropTypes.oneOfType([PropTypes.string, PropTypes.array, PropTypes.number]),
