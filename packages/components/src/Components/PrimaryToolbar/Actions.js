...
export const overflowActionsMapper = (action, key) => (
    <DropdownItem
        ...
        component={(action.props && action.props.component) || React.isValidElement(action.label || action) ? 'div' : 'button'}
        ...
        return (
            <Fragment>
                ...
                {((actions && actions.length > 0) || overflowActions.length > 0) && (
                    <DataToolbarItem className={`${actions.length <= 1 ? 'ins-m-actions--empty' : ''} ins-c-primary-toolbar__actions pf-m-spacer-sm`}>
                        <Dropdown
                            {...dropdownProps}
                            isOpen={isOpen}
                            isPlain
                                ...
                                ...(firstAction
                                    ? [
                                          <DropdownItem
                                              ...
                                              className={`ins-c-primary-toolbar__first-action ${(firstAction.props && firstAction.props.className) ||
                                                  ''}`}
                                          />
                                      ]
                                    : []),
