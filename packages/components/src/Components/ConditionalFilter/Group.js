...
class Group extends Component {
            ...
            .map(({ value, isChecked, onClick, label, props: itemProps, id, ...item }, key) => (
                <SelectOption
                    ...
                    onClick={(e) => {
                        ...
                        const props = [e, clickedGroup, clickedItem, groupValue || groupKey, value || key];
                ...
                >
                    {type === groupType.checkbox && (
                        <Checkbox
                            {...itemProps}
                            label={label}
                            isChecked={isChecked || this.isChecked(groupValue || groupKey, value || key) || false}
    ...
    render() {
        ...
        return (
            <Fragment>
                {!filterItems || (filterItems && filterItems.length <= 0) ? (
                    <Text {...this.props} value={`${selected}`} />
                ) : (
                    <Select
                        className={className}
