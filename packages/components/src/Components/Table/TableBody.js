...
class TableBody extends Component {
    ...
    createArrow(row, key) {
        return (
            <td className="pf-c-table__toggle pf-m-shrink" role="gridcell">
    ...
    render() {
        const { className, rows, hasCheckbox, onItemSelect, onColClick, onRowClick, expandable, onExpandClick, ...props } = this.props;
        return (
            <tbody {...props}>
                {rows &&
                    Object.keys(rows).map((oneKey) => this.createRow(rows[oneKey], rows[oneKey].hasOwnProperty('id') ? rows[oneKey].id : oneKey))}
            </tbody>
        );
    }
}
