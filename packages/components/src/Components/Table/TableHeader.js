...
class TableHeader extends Component {
    ...
    onHeadClick(event, key) {
        ...
        if (this.props.sortBy.index === key) {
            direction = this.props.sortBy && this.props.sortBy.direction === SortDirection.up ? SortDirection.down : SortDirection.up;
        } else {
    ...
    renderCol(col, key, hasSort = true) {
        return (
            <React.Fragment>
                {col}
                {hasSort && col && <span className="pf-c-table__sort-indicator">{this.createIcon(key, SortDirection.up)}</span>}
            </React.Fragment>
        );
    }
    ...
    createCheckbox() {
        ...
        return (
            ...
            >
                <input type="checkbox" className="pf-c-check" />
            </th>
        );
    }

    render() {
        const { className, onSort, sortBy, cols, hasIcon, hasCheckbox, onSelectAll, expandable, ...props } = this.props;
