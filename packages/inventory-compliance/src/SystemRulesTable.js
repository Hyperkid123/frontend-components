...
class SystemRulesTable extends React.Component {
    ...
    currentRows = (page, itemsPerPage, rowsRefIds) => {
        ...
        if (rows.length < itemsPerPage * 2) {
            itemsPerPage = rows.length / 2;
        }
    ...
    ruleTitleCell = (title, identifier) => (
        <Stack>
    ...
    columnsToTableHeaders = (columns, profile, { title, severity, compliant, identifier, remediationAvailable }) =>
        columns.map((column) => {
            ...
            if (column.title === 'Rule') {
                ...
                tableHeader = {
                    title: compliant ? <CheckCircleIcon className="ins-u-passed" /> : <ExclamationCircleIcon className="ins-u-failed" />,
                    original: compliant
                };
    ...
    rulesToRows = (profileRules) => {
        const refIds = {};
        const profiles = {};
        const rows = flatMap(profileRules, (profileRule) =>
            flatMap(profileRule.rules, (rule, key) => {
                ...
                return [this.calculateParent(profileRule, rule), this.calculateChild(rule, key)];
            })
    ...
    selectedRules = () => {
        ...
        return currentRows
            .filter((row) => row.selected && row.cells[columnIndices.REMEDIATIONS_COLUMN].original)
            .map((row) => ({
    ...
    onSort = (_event, index, direction) => {
        ...
        const sortedRows = this.uncompressRows(
            this.compressRows(rows).sort((a, b) => {
    ...
    filterBy = (attribute, rows, column) => {
        ...
        rows.forEach((row, i) => {
            if (
                this.isParent(row) &&
                String(row.cells[column].original)
                    .toLowerCase()
                    .match(String(attribute).toLowerCase())
            ) {
        ...
        if (searchTerm) {
            ...
            rows.forEach((row, i) => {
                ...
                const identifierMatches =
                    !this.isParent(row) && lowerString(row.cells[columnIndices.TITLE_COLUMN].originalIdentifier).match(searchTerm);
                const referencesMatch =
                    !this.isParent(row) && lowerString(row.cells[columnIndices.TITLE_COLUMN].originalReferences).match(searchTerm);
    ...
    recomputeParentId = (rows) => this.uncompressRows(this.compressRows(rows));
    ...
    updateFilter = (hidePassed, severity, policy) => {
        ...
        const filteredRows = this.filteredRows(passedRows, severityRows, policyRows, searchRows, hidePassed, severity, policy, searchTerm);
                ...
                <React.Fragment>
                    <TableToolbar>
                        <Level gutter="md">
                            ...
                            <LevelItem>{rows.length / 2} results</LevelItem>
                            {remediationsEnabled && (
                                <LevelItem>
