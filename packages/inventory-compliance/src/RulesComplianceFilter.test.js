...
describe('RulesComplianceFilter component', () => {
    ...
    it('should render filter', () => {
        const wrapper = shallow(<RulesComplianceFilter availablePolicies={policies} />);
        expect(toJson(wrapper)).toMatchSnapshot();
    });

    it('should not allow to filter if there is only one policy', () => {
        const wrapper = shallow(<RulesComplianceFilter availablePolicies={[policies[0]]} />);
        wrapper.instance();
    ...
    it('should not display the "Passed" filter if disabled', () => {
        const wrapper = shallow(<RulesComplianceFilter showPassFailFilter={false} />);
        wrapper.instance();
