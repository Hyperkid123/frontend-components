...
class CardSelect extends React.Component {
    ...
    handleMulti = (value) =>
        this.state.selectedValues.includes(value)
    ...
    handleClick = (value) => {
        this.isMulti ? this.handleMulti(value) : this.handleSingle(value);
    };
    ...
    prepareCards = () =>
        this.props.options
            .map((option) => this.props.mutator(option, this.props.formOptions))
            .map(({ value, label, isDisabled }) => {
                    ...
                    <GridItem sm={6} md={4} key={value}>
                        ...
                        >
                            <div className={disabled ? 'disabled' : ''}>
                                <CardHeader className={Component ? 'text-elipsis' : ''}>{label}</CardHeader>
                                <CardBody>
                                    <Bullseye>{Component && <Component size="xl" />}</Bullseye>
                                </CardBody>
                            </div>
                        </Card>
                    </GridItem>
    ...
    render() {
        ...
        return (
            ...
            >
                <Grid gutter="md">{this.prepareCards()}</Grid>
                <br />
            </FormGroup>
        );
    }
...
const CardSelectProvider = ({ FieldProvider, ...rest }) => (
    <FieldProvider {...rest}>{(props) => <CardSelect {...props} name={props.input.name} />}</FieldProvider>
);
