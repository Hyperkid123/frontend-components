...
describe('CardSelect component', () => {
    ...
    beforeEach(() => {
        ...
        initialProps = {
            FieldProvider,
            options: [{ value: 'ops', label: 'openshift' }, { value: 'aws', label: 'aws' }, { label: 'Choose one (this should not be displayed)' }],
            input: {
                name: 'card-select',
    ...
    it('should render correctly with mutator and it passes formOptions', () => {
        const wrapper = mount(
            <CardSelect {...initialProps} mutator={({ label, value }, formOptions) => ({ label: `AAA-${label}-${formOptions.something}`, value })} />
        );
    ...
    it('should change by pressing enter single select', () => {
        ...
        wrapper
            .find(Card)
            .last()
            .simulate('keypress', { charCode: 32, preventDefault: preventDefaultMock });
