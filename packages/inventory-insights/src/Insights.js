...
class InventoryRuleList extends Component {
    ...
    fetchKbaDetails = async (reportsData) => {
        const { filters, searchValue } = this.state;
        const kbaIds = reportsData
            .map((report) => report.rule.node_id)
            .filter((x) => x)
            .join(` OR `);
                ...
                const reportRow = [
                    {
                        ...
                        cells: [
                            ...
                            {
                                title: <div key={key}>{moment(rule.publish_date).fromNow()}</div>
                            },
                            {
                                title: (
                                    <div className="pf-m-center" key={key} style={{ verticalAlign: 'top' }}>
                                        <Battery label="Total risk" labelHidden severity={rule.total_risk} />
                                    </div>
                                )
                            },
                            {
                                title: (
                                    <div className="pf-m-center " key={key}>
                                        {resolution.has_playbook ? <CheckCircleIcon className="successColorOverride" /> : 'No'}
                                    </div>
                                )
                            }
                        ]
    ...
    processRemediation = (selectedAnsibleRules) => {
        ...
        const issues = playbookRows.map((r) => ({ id: `advisor:${r.rule.rule_id}`, description: r.rule.description }));

        return issues.length ? { issues, systems: [this.props.entity.id] } : false;
    };
                        ...
                        <MessageState
                            ...
                            text={
                                <span key="satellite managed system">
                                    Insights results can not be displayed for this host, as the &quot;Hide Satellite Managed Systems&quot; setting has
                                    been enabled by an org admin.
                                    <br />
...
export default connect(mapStateToProps, mapDispatchToProps)(InventoryRuleList);
